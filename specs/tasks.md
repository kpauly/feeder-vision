...
- [ ] **C7. Cached scan results**
  - [x] Persist per-folder scan results (path hash) under AppData, including manual edits
  - [x] Validate cache (file count/mtime/size); load cached rows when valid
  - [x] Keep Scan enabled to refresh and overwrite cache
